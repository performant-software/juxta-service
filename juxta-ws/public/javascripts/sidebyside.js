...
function initializeSideBySide() {
    ...
    $("#side-by-side").data("connections", new Array());
...
function initMouseWheelScrollHandling() {
...
function initWitnessScrollHandling() {
...
function getMaxWitnessHeight() {
    return $("#side-by-side").data("maxWitnessHeight");
}

function highlightDiff( diffEle ) {
...
function alignOnDiff(diffEle) {
    ...
    if(conn != null) {
        if(side == "left") {
            ...
            var delta = (conn.left - conn.right);
 ...
 */
function findConnection( tgtPos, side ) {
    var connections = $("#side-by-side").data("connections");
    for ( var i=0; i<connections.length; i++ ) {
        var conn = connections[i];
        if ( side == "right" ) {
...
function initCanvas() {
    ...
    $("#scroll-mode-img").css({"top": (top+1) +"px", "left": (imgLeft)+"px" });
 ...
 */
function renderAlignmentGutter() {
    ...
    var hdrHeight = $(".header").outerHeight();
    var defaultHeight = parseInt($('.witness-text').css('line-height'))-3;
    ...
    var bracketAttribs = {stroke: '#181', 'stroke-width': 0.5, 'stroke-linejoin': 'round'};
...
function renderTranspositionsGutter(hdrHeight, defaultHeight ) {
    ...
    var outline = {stroke: '#000', 'stroke-width': 5};
    var fill = {stroke: '#CCC', 'stroke-width': 3};
    var connections = $("#side-by-side").data("connections");
...
function renderConnections() {
    ...
    var connAttribs = {stroke: '#050', 'stroke-width': 0.5,'stroke-dasharray':'- '};
    var faded1 = {stroke: '#8a8', 'stroke-width': 0.5, 'stroke-dasharray':'- '};
    var faded2 = {stroke: '#beb', 'stroke-width': 0.5, 'stroke-dasharray':'- '};
    var moveLink = {stroke: '#f00', 'stroke-width': 0.5, 'stroke-dasharray':'- '};
    var litConnAttribs = {stroke: '#4ECE4E', 'stroke-width': 2};
    ...
    for ( var i=0; i<connections.length; i++) { 
        ...
        if ( conn.type == "transposition" ) {
            var pathStr = "M0,"+leftPos+"L"+width+","+rightPos;
            var line = connectionPaper.path( pathStr );
            line.attr(moveLink);
            continue;
        }
...
function initDocumentHeight() {
    ...
    if ( isEmbedded() == false ) { 
        ...
        var maxH = $(".content-scroller").height();
        var extraH = $(".header").outerHeight();
        newH = maxH - extraH;
    } else {
